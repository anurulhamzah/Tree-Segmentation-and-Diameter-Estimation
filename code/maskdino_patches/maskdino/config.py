...
def add_maskdino_config(cfg):
    ...
    cfg.INPUT.USE_DEPTH = False       # load depth channel alongside RGB
    cfg.INPUT.DEPTH_DIR = ""          # path to depth PFM files
    cfg.INPUT.MOSAIC_PROB = 0.0       # prob. 2x2 mosaic crowd-aug (coco_instance_lsj_dbh_mosaic mapper)
    cfg.INPUT.COPY_PASTE_PROB = 0.0   # prob. copy-paste crowd-aug (skala asli; CopyPasteDBHDatasetMapper)
    cfg.INPUT.COPY_PASTE_MAX_DONORS = 2  # jumlah maks gambar donor per sampel copy-paste
    ...
    # Per-class classification loss weight (len == num_classes, empty = uniform 1.0 for all classes).
    # Applied to SetCriterion.empty_weight[:num_classes] post-construction; index -1 (no-object)
    # still controlled separately by NO_OBJECT_WEIGHT above.
    cfg.MODEL.MaskDINO.PER_CLASS_WEIGHT = []
    ...
    cfg.MODEL.MaskDINO.DBH_WEIGHT = 0.0  # DBH regression head (0.0 = disabled)
    ...
    cfg.MODEL.SWIN.IN_CHANS = 3  # 3=RGB, 4=RGBD

    # focalnet backbone
    cfg.MODEL.FOCAL = CN()
    cfg.MODEL.FOCAL.modelname = "focalnet_L_384_22k"
    cfg.MODEL.FOCAL.out_features = ["res2", "res3", "res4", "res5"]
    cfg.MODEL.FOCAL.in_chans = 3  # 3=RGB, 4=RGBD (diteruskan ke FocalNet.__init__ via D2FocalNet)
