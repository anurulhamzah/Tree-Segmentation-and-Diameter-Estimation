...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        # DBH regression head: predicts log1p(dbh_mm) per query.
        # Input: [query_embedding (hidden_dim) | class_probs (num_classes)] so the head
        # can learn per-species DBH distributions rather than predicting a single mean.
        self.dbh_embed = nn.Sequential(
            nn.Linear(hidden_dim + num_classes, hidden_dim // 2),
            nn.ReLU(),
            nn.Linear(hidden_dim // 2, 1),
        )
        nn.init.constant_(self.dbh_embed[-1].weight.data, 0)
        # Init bias to population-mean log1p(DBH) (~32mm weighted mean across all species)
        # instead of 0, so early training starts near the data distribution.
        nn.init.constant_(self.dbh_embed[-1].bias.data, 3.5)
    ...
    def forward(self, x, mask_features, masks, targets=None):
        ...
        if self.two_stage:
            ...
            outputs_class, outputs_mask, outputs_dbh = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features)
            ...
            interm_outputs['pred_dbh'] = outputs_dbh
        ...
        if self.initial_pred:
            outputs_class, outputs_mask, outputs_dbh = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, self.training)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_dbh.append(outputs_dbh)
        ...
        for i, output in enumerate(hs):
            outputs_class, outputs_mask, outputs_dbh = self.forward_prediction_heads(output.transpose(0, 1), mask_features, self.training or (i == len(hs)-1))
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_dbh.append(outputs_dbh)
        ...
        out = {
            ...
            'pred_dbh': predictions_dbh[-1],
            'aux_outputs': self._set_aux_loss(
                predictions_class if self.mask_classification else None,
                predictions_mask, out_boxes, predictions_dbh,
            )
        }
        if self.two_stage:
    ...
    def forward_prediction_heads(self, output, mask_features, pred_mask=True):
        ...
        # Class-conditioned DBH: concatenate soft class probabilities so the head
        # can learn per-species trunk diameter distributions.
        class_probs = torch.softmax(outputs_class.detach(), dim=-1)  # (B, Q, C)
        dbh_input = torch.cat([decoder_output, class_probs], dim=-1)  # (B, Q, hidden+C)
        pred_dbh = self.dbh_embed(dbh_input).squeeze(-1)  # (B, Q)

        return outputs_class, outputs_mask, pred_dbh

    @torch.jit.unused
    def _set_aux_loss(self, outputs_class, outputs_seg_masks, out_boxes=None, out_dbh=None):
        ...
        if out_boxes is None:
            ...
            return [
                {"pred_logits": a, "pred_masks": b, "pred_boxes": c, "pred_dbh": d}
                for a, b, c, d in zip(outputs_class[:-1], outputs_seg_masks[:-1], out_boxes[:-1], out_dbh[:-1])
            ]