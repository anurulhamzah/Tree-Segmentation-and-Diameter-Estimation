...
class SetCriterion(nn.Module):
    ...
    def loss_dbh(self, outputs, targets, indices, num_masks):
        """Smooth-L1 regression for DBH in log-space.
        Skips instances where gt_dbh == 0 (species without a measurable trunk).
        pred_dbh is a raw scalar per query; target is log1p(dbh_mm).
        """
        if "pred_dbh" not in outputs or not any("dbh" in t for t in targets):
            return {"loss_dbh": torch.as_tensor(0.0, device=next(iter(outputs.values())).device)}
        src_idx = self._get_src_permutation_idx(indices)
        src_dbh = outputs["pred_dbh"][src_idx]  # (N,)
        tgt_dbh = torch.cat([t["dbh"][J] for t, (_, J) in zip(targets, indices)])  # (N,) mm
        valid = tgt_dbh > 0
        num_valid = valid.sum().clamp(min=1)
        loss = F.smooth_l1_loss(
            src_dbh[valid],
            torch.log1p(tgt_dbh[valid]),
            reduction="sum",
        ) / num_valid
