...
class MaskDINO(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        dbh_weight = cfg.MODEL.MaskDINO.get("DBH_WEIGHT", 0.0)
        # building matcher
        ...
        if dbh_weight > 0:
            weight_dict["loss_dbh"] = dbh_weight
    ...
    def forward(self, batched_inputs):
        ...
        if batched_inputs[0].get("depth") is not None:
            # RGBD mode: depth [0,1] → [0,255] then concat as 4th channel
            depths = [x["depth"].to(self.device).unsqueeze(0).mul(255.0) for x in batched_inputs]
            images = [torch.cat([img, d], dim=0) for img, d in zip(images, depths)]
    ...
    def prepare_targets(self, targets, images):
        ...
        for targets_per_image in targets:
            ...
            if targets_per_image.has("gt_dbh"):
                entry["dbh"] = targets_per_image.gt_dbh
