"""
Hybrid Trunk ROI DBH Training — V6 (dataset v2, strip_rows=5)
==============================================================
Perbedaan dari V5:

  1. Dataset bersih v2 (BARU):
       - Eksklusikan LeechVine (cat_id=11, liana — DBH tidak terdefinisi)
       - max_depth: 10m → 20m
       - Hasilnya: 6,344 train instance (vs ~4,606 non-LeechVine di v1, +38%)
       - Per-spesies: Mango=86, AliiFig=728, BangaloPalm=3451,
                      Fern=877, RubberFig=234, Umbrella=968
       - Lihat dokumen: Report_DBH_Species_Selection.html

  2. LeechVine dihapus dari CAT_LOG_MEAN/STD dan SPECIES_WEIGHT
     (v1 stats yang salah masuk — liana, 66% DBH=0)

  3. SPECIES_WEIGHT diperbarui berdasarkan N aktual dataset v2:
       Mango ×20, AliiFig ×4.7, BangaloPalm ×1.0 (baseline),
       Fern ×3.9, RubberFig ×14.7, Umbrella ×3.6

  4. strip_rows=5 dipertahankan (best dari V5)

Jalankan:
  nohup /scratch2/pr65/anur0018/conda_envs/maskdino/bin/python -u \\
      scripts/finetune_trunk_roi_dbh_hybrid_v6.py \\
      ...
      --output-dir /scratch2/pr65/anur0018/maskdino_output/FocalNet_L_trunk_roi_dbh_hybrid_v6_json_v2_strip5_20k \\
      > logs/trunk_roi_dbh_hybrid_v6_json_v2_strip5_20k_nohup.log 2>&1 &
"""
...
logger = logging.getLogger("finetune_hybrid_v6")

# Per-species log1p(DBH_mm) statistics — dihitung dari dataset v2 train
# LeechVine (11) dihapus: liana, DBH tidak terdefinisi dalam standar forestry
CAT_LOG_MEAN = {
    4:  5.5160,   # Mango       N=86
...
CAT_NAMES = {
    4:'Mango', 8:'AliiFig', 9:'BangaloPalm',
    10:'Fern', 12:'RubberFig', 13:'Umbrella'
}

# Per-species weights — inverse frequency, baseline=BangaloPalm (N=3451)
# Mango capped at 20 (3451/86=40.1 terlalu ekstrem)
SPECIES_WEIGHT = {
...
def main(args):
    ...
    # ── Hybrid head v6: strip_rows=5 (sama dengan V5) ───────────────────────
    ...
    logger.info(f"HybridTrunkROIDBHHead v6: {n_params:,} params, strip_rows=5, dropout=0.3")
    ...
    metrics_path = os.path.join(args.output_dir, "metrics_hybrid_v6.json")
    metrics_log  = []
    ...
    logger.info(f"V6 training: {args.max_iter} iters, lr={args.lr}, wd=5e-4, dropout=0.3, strip_rows=5")
    logger.info("Dataset v2: max_depth=20m, LeechVine excluded, 6 spesies, N_train=6344")
    logger.info("SPECIES_WEIGHT (inverse freq, BangaloPalm=1.0):")
    for cid, w in sorted(SPECIES_WEIGHT.items()):
        logger.info(f"  {CAT_NAMES.get(cid,cid):<14} ×{w:.1f}  N={[86,728,3451,877,234,968][sorted(SPECIES_WEIGHT.keys()).index(cid)]}")
    ...
    for iteration in range(1, args.max_iter + 1):
        ...
        if iteration % args.eval_period == 0 or iteration == args.max_iter:
            latest_ckpt = os.path.join(args.output_dir, "hybrid_v6_latest.pth")
            ...
            if "N" in metrics:
                ...
                if r2 > best_r2:
                    ...
                    ckpt = os.path.join(args.output_dir, "hybrid_v6_best.pth")
    ...
    final_ckpt = os.path.join(args.output_dir, "hybrid_v6_final.pth")
...
if __name__ == "__main__":
    ...
    logger = setup_logger(name="finetune_hybrid_v6")
