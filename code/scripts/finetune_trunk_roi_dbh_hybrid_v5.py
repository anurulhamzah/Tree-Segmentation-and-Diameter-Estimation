"""
Hybrid Trunk ROI DBH Training — V5 (strip_rows=5)
==================================================
Perbedaan dari V4 (finetune_trunk_roi_dbh_hybrid_v4.py):

  1. strip_rows=5 (BARU): strip feature diperluas dari ±3 ke ±5 feature rows
     (stride=4, jadi ±20px di image space).
       Motivasi: V4 (strip_rows=3) memberi lompatan besar vs V3 (strip_rows=1):
       avg per-spesies −0.375 → −0.248 (+0.127). Tren menunjukkan konteks lebih
       lebar masih membantu; strip_rows=5 adalah langkah logis berikutnya.
       Risiko: window terlalu lebar bisa masuk crown region — akan terlihat dari
       apakah gain melambat (diminishing returns) atau justru turun.

  2. Per-species weighted loss tetap aktif (identik V3/V4)
...
Jalankan:
  nohup /scratch2/pr65/anur0018/conda_envs/maskdino/bin/python -u \\
      scripts/finetune_trunk_roi_dbh_hybrid_v5.py \\
      ...
      --output-dir /scratch2/pr65/anur0018/maskdino_output/FocalNet_L_trunk_roi_dbh_hybrid_v5_strip5_20k \\
      > logs/trunk_roi_dbh_hybrid_v5_strip5_20k_nohup.log 2>&1 &
"""
...
logger = logging.getLogger("finetune_hybrid_v5")
...
def main(args):
    ...
    # ── Hybrid head v5: strip_rows=5 (naik dari 3) ──────────────────────────
    ...
    logger.info(f"HybridTrunkROIDBHHead v5: {n_params:,} params, strip_rows=5, dropout=0.3")
    ...
    metrics_path = os.path.join(args.output_dir, "metrics_hybrid_v5.json")
    metrics_log  = []
    ...
    logger.info(f"V5 training: {args.max_iter} iters, lr={args.lr}, wd=5e-4, dropout=0.3, strip_rows=5")
    ...
    for iteration in range(1, args.max_iter + 1):
        ...
        if iteration % args.eval_period == 0 or iteration == args.max_iter:
            latest_ckpt = os.path.join(args.output_dir, "hybrid_v5_latest.pth")
            ...
            if "N" in metrics:
                ...
                if r2 > best_r2:
                    ...
                    ckpt = os.path.join(args.output_dir, "hybrid_v5_best.pth")
    ...
    final_ckpt = os.path.join(args.output_dir, "hybrid_v5_final.pth")
...
if __name__ == "__main__":
    ...
    logger = setup_logger(name="finetune_hybrid_v5")
