#!/usr/bin/env python3
"""HTML comparison V2 vs V4 for 81 target images (main tree annotation)."""
...
OUT_HTML = '/home/anur0018/pr65_scratch2/anur0018/tree_classification/compare_78_v2_vs_v4.html'
...
print(f'V2: {sum(len(v) for v in v2_id2anns.values())} anns | '
      f'V4: {sum(len(v) for v in v4_id2anns.values())} anns')
...
for stem in STEMS:
    ...
    v4_main = find_main_ann(v4_anns, inst_flat, H, W)

    v2_main_m = v2_main[1] if v2_main else None
    v4_main_m = v4_main[1] if v4_main else None
    v2_area   = v2_main[0]['area'] if v2_main else 0
    v4_area   = v4_main[0]['area'] if v4_main else 0
    gain      = v4_area - v2_area
    ratio     = v4_area / max(v2_area, 1)
    ...
    p_v4   = img_to_b64(make_overlay(rgb, v4_main_m, other_masks(v4_anns, v4_main)))
...
<div class="card">
  ...
  <div class="panels">
    ...
    <div class="panel"><img src="data:image/png;base64,{p_v4}">
      <div class="lbl">V4 — Exact Match ({v4_area:,}px)</div></div>
  </div>
</div>""")
    print(f'{stem}: {v2_area:,} → {v4_area:,} ({gain:+,}px, {ratio:.2f}x)')

if missing:
    print(f'\nMissing/skip: {missing}')

html = f"""<!DOCTYPE html><html><head><meta charset="utf-8">
<title>V2 vs V4 — 81 Target Images</title>
<style>
...
<h2>Perbandingan V2 (NN) vs V4 (Distance Exact Match) — {len(cards)} gambar
