"""
Membuat COCO JSON bersih untuk training DBH head — versi 2 (kriteria diperbarui).

Perubahan dari v1:
  - MAX_DEPTH: 10m → 20m  (memanfaatkan instance yang sebelumnya terbuang)
  - Tambah LeechVine (cat_id=11) ke daftar eksklusif (liana, DBH tidak terdefinisi)
  - Hapus MAX_TRUNK_FRAC filter (trunk_px/W_img ratio terlalu ketat, tidak perlu)
  - Proses semua 3 split: train, val, test

Level 0 — Seleksi spesies (hard-coded):
  Include: 4 (Mango), 8 (AliiFig), 9 (BangaloPalm), 10 (Fern),
           12 (RubberFig subset DBH<150cm), 13 (Umbrella)
  Exclude: 1,2,3,5,6,7 (preset GT perkebunan) + 11 (LeechVine, liana)

Level 1 — GT-based (tanpa load file):
  - Exclude DBH = 0
  - Exclude RubberFig DBH ≥ 150cm (bimodal, kelompok besar tidak feasible)

Level 2 — Geometric feasibility (load PFM + decode RLE):
  - depth di row 1.3m ∈ [1, 20m)  ← relaksasi dari 10m
  - |world_height - 1.3| ≤ 0.5m
  - trunk_px ≥ 5

Output: instances_{train,val,test}_dbh_clean.json di OUT_DIR
"""
...
# ── Level 0: spesies yang dieksklusikan ──────────────────────────────────────
# 1-7 preset GT perkebunan (R²<0.05 gt vs geometric), 11 liana/vine
EXCLUDE_SPECIES = {1, 2, 3, 5, 6, 7, 11}

# ── Level 1 params ────────────────────────────────────────────────────────────
RF_CAT_ID     = 12
RF_DBH_MAX_CM = 150.0   # RubberFig: hanya subset kecil (43–75cm kelompok, batas 150cm)
MIN_DBH_CM    = 0.0

# ── Level 2 params ────────────────────────────────────────────────────────────
MIN_DEPTH    = 1.0    # meter
MAX_DEPTH    = 20.0   # meter  ← RELAXED dari 10.0
...
def find_row_1p3m(mask: np.ndarray, depth_raw: np.ndarray,
                  ...
                  tol_wh: float = TOL_WH,
                  depth_tol: float = 0.3):
    """Cari row paling dekat world height 1.3m. Returns (trunk_px, d_trunk, world_h) atau None."""
...
if __name__ == "__main__":
    ...
    OUT_DIR   = f"{BASE}/annotation_inst/filtered_rle_f1000_repaired_v4_stratified_dbh_clean_v2"

    os.makedirs(OUT_DIR, exist_ok=True)
    print(f"Output dir: {OUT_DIR}")
    print(f"MAX_DEPTH  = {MAX_DEPTH}m  (v1: 10m)")
    print(f"EXCLUDE    = {sorted(EXCLUDE_SPECIES)}  (v1: {{1,2,3,5,6,7}}, tambah LeechVine=11)")
    ...
    print(f"\n{'='*60}")
    print(f"DONE — clean JSONs v2 siap untuk training DBH head V6")
    print(f"  Train: {n_train}  Val: {n_val}  Test: {n_test}  Total: {n_train+n_val+n_test}")
    print(f"  (v1 clean: ~4,763 train — ekspektasi v2: ~16,000+ train)")
    print(f"{'='*60}")
