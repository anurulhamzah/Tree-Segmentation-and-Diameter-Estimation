"""
Hybrid Trunk ROI DBH Training — V4 (strip_rows=3)
==================================================
Perbedaan dari V3 (finetune_trunk_roi_dbh_hybrid_v3.py):

  1. strip_rows=3 (BARU): strip feature di sekitar row 1.3m diperluas dari
     ±1 ke ±3 feature rows (stride=4, jadi ±12px di image space).
       Motivasi: AliiFig dan Umbrella punya trunk lebih tebal; konteks
       ±1 row mungkin tidak cukup menangkap variasi visual trunk mereka.
       V2/V3 bottleneck diduga di feature representation, bukan objective.
     Input dim naik dari 207 ke 207 (geometric+species tetap 15 dim;
     hanya cara strip_feat di-average yang berubah — output tetap 192-dim).

  2. Per-species weighted loss tetap aktif (dari V3):
     Mango×20, AliiFig×6, Fern×5, LeechVine×18, RubberFig×21, Umbrella×5

  3. Semua hiperparameter lain tetap: dropout=0.3, wd=5e-4, lr=1e-4,
     cosine decay, 20k iter, eval_period=1000.

Jalankan:
  nohup /scratch2/pr65/anur0018/conda_envs/maskdino/bin/python -u \\
      scripts/finetune_trunk_roi_dbh_hybrid_v4.py \\
      ...
      --output-dir /scratch2/pr65/anur0018/maskdino_output/FocalNet_L_trunk_roi_dbh_hybrid_v4_strip3_20k \\
      > logs/trunk_roi_dbh_hybrid_v4_strip3_20k_nohup.log 2>&1 &
"""
...
logger = logging.getLogger("finetune_hybrid_v4")
...
# Per-species loss weights — identik dengan V3
SPECIES_WEIGHT = {
    4:  20.0,   # Mango:       2879/77  = 37.4 → cap 20
    8:   6.0,   # AliiFig:     2879/478 = 6.0
    9:   1.0,   # BangaloPalm: baseline
    10:  5.0,   # Fern:        2879/580 = 5.0
    11:  18.0,  # LeechVine:   2879/157 = 18.3
    12:  21.0,  # RubberFig:   2879/136 = 21.2
    13:   5.0,  # Umbrella:    2879/613 = 4.7
}
...
def main(args):
    ...
    # ── Hybrid head v4: strip_rows=3 (naik dari 1) ──────────────────────────
    trunk_head = HybridTrunkROIDBHHead(
        in_channels=192, hidden=256,
        strip_rows=3, num_species=13,
        dropout=0.3,
    ).to(device)
    n_params = sum(p.numel() for p in trunk_head.parameters())
    logger.info(f"HybridTrunkROIDBHHead v4: {n_params:,} params, strip_rows=3, dropout=0.3")
    ...
    metrics_path = os.path.join(args.output_dir, "metrics_hybrid_v4.json")
    metrics_log  = []
    ...
    logger.info(f"V4 training: {args.max_iter} iters, lr={args.lr}, wd=5e-4, dropout=0.3, strip_rows=3")
    ...
    logger.info("SPECIES_WEIGHT (invers frekuensi, BangaloPalm=1.0):")
    ...
    for iteration in range(1, args.max_iter + 1):
        ...
        if iteration % args.eval_period == 0 or iteration == args.max_iter:
            latest_ckpt = os.path.join(args.output_dir, "hybrid_v4_latest.pth")
            ...
            if "N" in metrics:
                ...
                if r2 > best_r2:
                    ...
                    ckpt = os.path.join(args.output_dir, "hybrid_v4_best.pth")
    ...
    final_ckpt = os.path.join(args.output_dir, "hybrid_v4_final.pth")
...
if __name__ == "__main__":
    ...
    logger = setup_logger(name="finetune_hybrid_v4")
