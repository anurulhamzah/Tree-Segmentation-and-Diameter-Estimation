"""
Hybrid Trunk ROI DBH Training — V3 (Per-Species Weighted Loss)
==============================================================
Perbedaan dari V2 (finetune_trunk_roi_dbh_hybrid_v2.py):

  1. Per-species loss weighting (BARU): weight invers-frekuensi per spesies
       Masalah V2: BangaloPalm (58.5% training data) mendominasi gradient
       → spesies lain (AliiFig, Fern, dll) hampir tidak belajar (R²≤0)
       Solusi: kalikan loss per-instance dengan weight invers jumlah data

     Weight (invers frekuensi relatif ke BangaloPalm, cap=21):
       Mango      ×20  (2879/77=37.4 → cap 20)
       AliiFig    × 6  (2879/478=6.0)
       BangaloPalm× 1  (baseline)
       Fern       × 5  (2879/580=5.0)
       LeechVine  ×18  (2879/157=18.3)
       RubberFig  ×21  (2879/136=21.2)
       Umbrella   × 5  (2879/613=4.7)

  2. Semua V2 lainnya tetap sama: clean JSON, ratio filter off, full eval,
     dropout 0.3, weight decay 5e-4, eval_period 1000

Jalankan:
  nohup /scratch2/pr65/anur0018/conda_envs/maskdino/bin/python -u \\
      scripts/finetune_trunk_roi_dbh_hybrid_v3.py \\
      ...
      > logs/hybrid_v3_weighted_20k_nohup.log 2>&1 &
"""
...
logger = logging.getLogger("finetune_hybrid_v3")
...
# Per-species loss weights — invers frekuensi relatif ke BangaloPalm (N=2879)
# N train: Mango=77, AliiFig=478, BangaloPalm=2879, Fern=580,
#          LeechVine=157, RubberFig=136, Umbrella=613
SPECIES_WEIGHT = {
...
def main(args):
    ...
    # ── Hybrid head v3: arsitektur sama dengan v2 ────────────────────────────
    ...
    logger.info(f"HybridTrunkROIDBHHead v3: {n_params:,} params, dropout=0.3")
    ...
    metrics_path = os.path.join(args.output_dir, "metrics_hybrid_v3.json")
    metrics_log  = []
    ...
    logger.info(f"V3 training: {args.max_iter} iters, lr={args.lr}, wd=5e-4, dropout=0.3")
    ...
    for iteration in range(1, args.max_iter + 1):
        ...
        # BARU V3: species_weights untuk koreksi dominansi BangaloPalm
        ...
        if iteration % args.eval_period == 0 or iteration == args.max_iter:
            latest_ckpt = os.path.join(args.output_dir, "hybrid_v3_latest.pth")
            ...
            if "N" in metrics:
                ...
                if r2 > best_r2:
                    ...
                    ckpt = os.path.join(args.output_dir, "hybrid_v3_best.pth")
    ...
    final_ckpt = os.path.join(args.output_dir, "hybrid_v3_final.pth")
...
if __name__ == "__main__":
    ...
    logger = setup_logger(name="finetune_hybrid_v3")
